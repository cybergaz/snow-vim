...
-- -------------------------------------------------------------
-- strings operation with backspace (require mini.ai)
-- -------------------------------------------------------------
...
vim.keymap.set("n", "<leader>tl", toggle_diagnostics, { desc = "Toggle Virtual Diagnostics Lines" })
...
-- -------------------------------------------------------------
local list_unsaved = function()
    local bufs = require("core.utils").unsaved_buffers()

    if #bufs == 0 then
        vim.notify("No unsaved buffers 🎉")
        return
    end
...
map("n", "<leader>ub", list_unsaved, { desc = "List unsaved buffers" })
map("n", "<leader>bb", list_unsaved, { desc = "List unsaved buffers" })
map("n", "<leader>m", list_unsaved, { desc = "List unsaved buffers" })
